        ...
        <header className="sticky top-0 z-50 w-full border-b border-brand-gray/20 bg-brand-black/95 backdrop-blur supports-[backdrop-filter]:bg-brand-black/60">
            <div className="container mx-auto px-4 h-16 flex items-center justify-between">
                ...
                <nav className="hidden md:flex items-center space-x-6 text-sm font-medium text-gray-300">
                    ...
                    <Link href="/contacto" className="px-4 py-2 bg-brand-yellow text-brand-black rounded-md font-bold hover:bg-yellow-400 transition-colors">
                        Contactar
                    </Link>
                </nav>
            ...
            {isMenuOpen && (
                <div className="md:hidden border-t border-brand-gray/20 bg-brand-black">
                    <nav className="flex flex-col px-4 py-4 space-y-4 text-sm font-medium text-gray-300">
                        <Link href="/" className="hover:text-brand-yellow transition-colors block" onClick={() => setIsMenuOpen(false)}>
                            Inicio
                        </Link>
                        <Link href="/#catalogo" className="hover:text-brand-yellow transition-colors block" onClick={() => setIsMenuOpen(false)}>
                            Catálogo
                        </Link>
                        <Link href="/contacto" className="inline-block px-4 py-2 bg-brand-yellow text-brand-black rounded-md font-bold hover:bg-yellow-400 transition-colors text-center" onClick={() => setIsMenuOpen(false)}>
                            Contactar
                        </Link>
                    </nav>
                </div>
